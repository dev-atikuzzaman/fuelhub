...
function DeadlineBadge({ deadline }) {
  ...
  if (days === 0) return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 20, background: '#fee2e2', color: '#dc2626', fontSize: 11.5, fontWeight: 700 }}>🔥 আজই শেষ দিন</span>;
  if (days <= 3) return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 20, background: '#fef3c7', color: '#d97706', fontSize: 11.5, fontWeight: 700 }}>⚠️ {days} দিন বাকি</span>;
  return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 20, background: '#f0f9ff', color: '#0284c7', fontSize: 11.5, fontWeight: 700 }}>📅 {days} দিন বাকি</span>;
}
            ...
            <div key={u.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                ...
                {u.user_id === currentUser.id && (
                  <button onClick={() => handleDelete(u.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--danger)', padding: 4, flexShrink: 0 }}>
      ...
      {showForm && (
        <div onClick={() => setShowForm(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16, backdropFilter: 'blur(4px)' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--bg-surface)', borderRadius: 22, padding: 22, width: '100%', maxWidth: 480, boxShadow: 'var(--shadow-lg)', animation: 'slideUp 0.3s ease', maxHeight: '85vh', overflowY: 'auto' }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 17, fontWeight: 800, color: 'var(--text-primary)' }}>নতুন আপডেট</h3>
