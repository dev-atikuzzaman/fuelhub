              ...
              <div key={note.id} style={{
                ...
                <div style={{ position: 'absolute', top: 10, right: 10, display: 'flex', gap: 4 }}>
                  <button onClick={() => { setEditingNote(note); setForm({ title: note.title, body: note.body, color: note.color }); setShowForm(true); }}
                    style={{ background: 'rgba(255,255,255,0.7)', border: 'none', cursor: 'pointer', color: '#475569', padding: 5, borderRadius: 8 }}>
                    <EditIcon width={13} height={13} />
                  </button>
                  <button onClick={() => handleDelete(note)}
                    style={{ background: 'rgba(255,255,255,0.7)', border: 'none', cursor: 'pointer', color: '#ef4444', padding: 5, borderRadius: 8 }}>
